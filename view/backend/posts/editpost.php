                    ...
                    <form method="post" action="index.php?page=editpost&id=<?= $editPost['postId'] ?>" enctype="multipart/form-data">
                        <fieldset class="form-group form-post row g-0">                
                            ...
                            <div class="form-field col-lg-12">
                                ...
                                <input type="text" name="title" id="title" class="form-control" value="<?= $this->issetSession('input','title') ? $this->getSession('input','title') : $editPost['title'] ?>">
                            </div>
                            <div class="form-field col-lg-12">
                                <label for="headline">Headline</label>
                                <input type="text" name="headline" id="headline" class="form-control" value="<?= $this->issetSession('input','headline') ? $this->getSession('input','headline') : $editPost['headline'] ?>">
                            </div>
                            
                            <div class="form-field col-lg-12">
                                <label for="exampleFormControlTextarea1" class="form-label">Content</label>
                                <textarea name="content" <?php if($this->getSession('auth','id') == $editPost['id']) : ?> id="content" <?php endif; ?> rows="8" class="form-control"><?= $this->issetSession('input','content') ?  $this->getSession('input','content') : $editPost['content'] ?></textarea>
                            </div>
                            <div class="form-field col-lg-12 d-flex flex-column mb-3">
                                <label for="formFile" class="form-label">Image</label>
                                <?php if(!empty($editPost['image']) && $editPost['image'] != NULL) : ?>
                                    <img src="assets/images/<?= $this->issetSession('input','image') ? $this->getSession('input','image') : $editPost['image']; ?>" alt="post-pic" width="200px;">
