                    ...
                    <form method="post" action="index.php?page=editpost&id=<?= $editPost['postId'] ?>" enctype="multipart/form-data">
                        <fieldset class="form-group form-post row g-0">                
                            ...
                            <div class="form-field col-lg-12">
                                ...
                                <?php if ($this->getSession('auth','id') != $editPost['user_id']) : ?> readonly <?php endif; ?> 
                                value="<?= $this->issetSession('input','title') ? $this->getSession('input','title') : $editPost['title'] ?>">
                            </div>
                            <div class="form-field col-lg-12">
                                <label for="headline">Headline</label>
                                <input type="text" name="headline" id="headline" class="form-control" 
                                <?php if ($this->getSession('auth','id') != $editPost['user_id']) : ?> readonly <?php endif; ?> 
                                value="<?= $this->issetSession('input','headline') ? $this->getSession('input','headline') : $editPost['headline'] ?>">
                            </div>
                            
                            <div class="form-field col-lg-12">
                                <label for="exampleFormControlTextarea1" class="form-label">Content</label>
                                <textarea name="content" rows="8" id="content" class="form-control" <?php if ($this->getSession('auth','id') != $editPost['user_id']) : ?> readonly <?php endif; ?>>
                                    <?= $this->issetSession('input','content') ?  $this->getSession('input','content') : $editPost['content'] ?>
                                </textarea>
                            </div>
                            <div class="form-field col-lg-12 d-flex flex-column mb-3">
                                <label for="formFile" class="form-label">Image</label>
                                <?php if(!empty($editPost['image']) && $editPost['image'] != NULL) : ?>
                                    <img src="assets/images/<?= $this->issetSession('input','image') ? $this->getSession('input','image') : $editPost['image']; ?>" 
                                    alt="post-pic" width="200px;">
                                    <?= $this->issetSession('image','name') ? basename($this->getSession('image','name')) : basename($editPost['image']); ?>
                                <?php else : ?>
                                    ...
                                    <option value="0">Select option</option>
                                        <?php if(count($selectTag)>0) : ?>
                                            <?php for ($i=0; $i<count($selectTag); $i++) : ?>
                                                <option value="<?= $selectTag[$i]['id']?>"
                                                    <?= isset($editPost['tag_id']) && $selectTag[$i]['id'] == $editPost['tag_id'] ? "selected" : "" ?>><?= $selectTag[$i]['tagname']?>
                                                </option>
                                            <?php endfor; ?>    
                                    <?php endif; ?>    
                                </select>
                            ...
                            <?php if ($this->issetSession('auth','role') && $this->getSession('auth','role') == 2): ?>
                                <div class="form-check">
                                    <input type="checkbox" name="status_post" class="form-check-input mt-3" value="2" 
                                    <?php if(isset($editPost['status_post']) && $editPost['status_post'] == 2) { ?><?= "checked='checked'"; } ?> id="checkbox">
