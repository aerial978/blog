                    ...
                    <form method="post" action="" enctype="multipart/form-data">
                        <fieldset class="form-group form-post row g-0">
                            ...
                            <div class="form-field col-lg-12">
                                ...
                                value="<?= $this->issetSession('input','title') ? $this->getSession('input','title') : "" ; ?>">
                            </div>
                            <div class="form-field col-lg-12">
                                <label for="headline">Headline</label>
                                <input type="text" name="headline" id="headline" class="form-control" 
                                value="<?= $this->issetSession('input','headline') ? $this->getSession('input','headline') : "" ; ?>">
                            </div>
                            <div class="form-field col-lg-12">
                            <label for="exampleFormControlTextarea1" class="form-label">Content</label>
                            <textarea name="content" id="content" class="form-control"><?= $this->issetSession('input','content') ? $this->getSession('input','content'): "" ; ?>
                            </textarea>      
                            </div>
                            <div class="form-field col-lg-12 file-input">
                                <label for="formFile" class="form-label">Image</label>
                                <input type="file" name="image" id="image" class="form-control" 
                                value="<?= $this->issetSession('input','image') ? $this->getSession('input','image') : "" ; ?>">
                            </div>
                            <div class="form-field col-lg-12">
                                ...
                                <select name="tagname" id="tagname" class="form-control">
                                    ...
                                    <?php if(count($selectTag)>0) : ?>
                                        <?php for ($i=0; $i<count($selectTag); $i++) : ?>
                                            <option <?= $this->issetSession('input','tagname') && $this->getSession('input','tagname') == $selectTag[$i]['id'] ? "selected" : "" ; ?> 
                                                value="<?= $selectTag[$i]['id'];?>"><?= $selectTag[$i]['tagname'];?>
                                            </option>
                                        <?php endfor; ?>    
                                    <?php endif; ?> 
                                </select>
                            ...
                            <div class="form-check">
                                <input type="checkbox" name="status_post" class="form-check-input mt-3" 
                                value="2" <?php if(isset($posts['status_post']) && $posts['status_post'] == 2) { ?><?= "checked='checked'"; } ?> id="checkbox">
